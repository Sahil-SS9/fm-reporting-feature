...
export const ReportHistoryTab: React.FC<ReportHistoryTabProps> = ({ 
  ...
  const handleDelete = (instanceId: string) => {
    ...
    setSortedInstances(prev => prev.filter(i => i.id !== instanceId));
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">
          Report History ({sortedInstances.length} instances)
        </h3>
                ...
                <TableCell>
                  {instance.filters && Object.keys(instance.filters).length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {Object.entries(instance.filters).slice(0, 2).map(([key, value]) => (
                        <Badge key={key} variant="secondary" className="text-xs">
                          {key.replace(/_/g, ' ')}: {Array.isArray(value) ? value.length : String(value)}
                        </Badge>
                      ))}
                      {Object.keys(instance.filters).length > 2 && (
                        <Badge variant="secondary" className="text-xs">
                          +{Object.keys(instance.filters).length - 2} more
                        </Badge>
                      )}
                    </div>
                  ) : (
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => handleEmail(instance)}>
